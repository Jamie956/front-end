...
const isValidKey = element => {
  ...
  return (
    ["string", "number", "boolean", "undefined"].includes(typeof element) ||
    isNull
  );
};
...
const arrayToKeyMirror = arr => {
  ...
  return arr.reduce((acc, key) => {
    if (!isValidKey(key)) {
      throw new MirrarrayError(
        "Invalid element contained within input array; each element must be either a string or number!"
      );
    }
    if (!nonOverlappingKey(key)) {
      throw new MirrarrayError(
        "Distinct elements in the input array are coercing to the same string!"
      );
    }
    acc[key] = key;
    return acc;
